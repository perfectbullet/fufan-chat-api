...
"""
如果历史记录存在，它会构建一个有效组合提示、大型语言模型 (LLM) 和结构化输出解析器 ( StrOutputParser ) 的序列，后跟检索器。
此顺序可确保最新问题在累积的历史数据中得到体现。
"""
...
"""
到目前为止，我们探索了如何将历史交互集成到应用程序逻辑中。然而，我们一直在手动处理聊天历史记录——为每次用户交互更新和插入它。
在强大的问答应用程序中，自动化此过程对于效率和可扩展性至关重要。
接下来看两个组件：
    - BaseChatMessageHistory：该组件用于存储聊天历史记录。
    - RunnableWithMessageHistory：这是 LCEL 链与 BaseChatMessageHistory 结合的包装器。
    它自动执行将聊天历史记录注入输入的过程，并在每次调用后更新它。
...
# Step 6. 此提示告诉模型接收聊天历史记录和用户的最新问题，然后重新表述问题，以便可以独立于聊天历史记录来理解问题。
# 明确指示模型不要回答问题，而是在必要时重新表述问题。
contextualize_q_system_prompt = """Given a chat history and the latest user question \
which might reference context in the chat history, formulate a standalone question \
which can be understood without the chat history. Do NOT answer the question, \
